(ns insulationcrew.governor
  "InsulationCrewGovernor — the independent safety/scope layer gating
  every job-site scheduling/logistics proposal an advisor may make for
  an insulation crew. The governor never dispatches hardware itself,
  never performs insulation-installation work on the job site, and
  never finalizes an insulation-installation-execution decision or
  overrides a site safety officer's judgment — those are permanently
  out of this actor's scope and remain a site safety officer's
  exclusive judgment (README's 'Robotics premise': this actor
  coordinates JOB-SITE SCHEDULING/LOGISTICS ONLY — it never performs
  insulation-installation work itself). Modeled on
  ...
  HARD invariants (:hard? true, ALWAYS :hold, never overridable):
    ...
    3. no-actuation           — proposal :effect must be :propose (the
                                ...
                                never performs insulation-installation
    ...
    5. scope-excluded action  — any proposal to directly finalize an
                                insulation-installation-execution
                                decision, or to override a site safety
                                ...
                                \"proceed with the insulation
                                installation\" / \"authorize the
                                insulation installation\" / \"override
                                the site safety officer's judgment\",
                                never as bare nouns like \"insulation\"
                                ...
                                crew operation for insulation task …\"
                                or \"…routed for site safety officer
  ...
  regardless of confidence):
    6. :op :flag-safety-concern (a particulate-exposure / confined-space
                                / ventilation concern always escalates
...
(def ^:private scope-excluded-ops
  #{:finalize-insulation-installation-decision
    :authorize-insulation-installation
    :proceed-with-insulation-installation
...
;; ("insulation", "particulate", "confined-space", "ventilation",
;; "safety") — so this can never match inside the mock advisor's own
;; default rationale text (which legitimately contains those bare
;; nouns, e.g. "insulation task" / "site safety officer review"). See
;; `governor-test/default-mock-advisor-proposals-never-self-trip-on-scope-exclusion`.
(def ^:private scope-excluded-phrases
  ["proceed with the insulation installation"
   "proceed with the insulation installation work"
   "finalize the insulation installation decision"
   "authorize the insulation installation"
  ...
  (let [{:keys [op rationale]} proposal]
    (cond-> []
      ...
      (conj {:rule :scope-excluded-action
             :detail "断熱施工の実行判断の確定・site safety officer の判断の上書きは、この actor の権限外 — 常に永続ブロック"}))))

(defn check
  "Assess a proposal against `request`/`context`/`proposal` and a
  `store` implementing `insulationcrew.store/Store`. Pure — never
