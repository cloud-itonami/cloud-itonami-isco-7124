(ns insulationcrew.advisor
  "Insulation Crew Advisor — proposing a job-site scheduling/logistics
  coordination operation (log a work record, schedule a crew
  operation, flag a safety concern, coordinate an insulation-materials
  supply order) from a crew roster, job-site registration and
  safety-reporting policy. Swappable mock/llm; the advisor ONLY
  proposes — `insulationcrew.governor` independently gates every
...
(defn- rationale-for [op worker-id site-id hazard-type]
  (case op
    ...
    (str "scheduled crew operation for insulation task at site " site-id)
...
(def ^:private system-prompt
  "You are an insulation-crew job-site scheduling/logistics coordination
   ...
   propose to directly finalize an insulation-installation-execution
   decision, or to override a site safety officer's judgment — those
   are always out of this actor's scope; it coordinates job-site
   scheduling/logistics only and never performs insulation-installation
   work or overrides site safety judgment itself. Safety concerns
   (particulate exposure, confined-space, ventilation) always require
